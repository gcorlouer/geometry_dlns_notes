...
\begin{proposition}[Associated bundle model for $\Omega_r$]\label{prop:Omega_r-associated-bundle}
Let $\Omega_r$ be as in Definition~\ref{def:Omega_r}, and let $H_r$ be as in
Definition~\ref{def:Hr}. Define an action of $H_r$ on $G\times \mathcal Z$ by
\begin{equation}\label{eq:Hr-action-on-GxZ}
(g,Z)\cdot h \;:=\; \bigl(g h,\; h^{-1}\cdot Z\bigr),
\end{equation}
where for $h\in H_r$ written as in \eqref{eq:Hr-form},
\begin{equation}\label{eq:Hr-action-on-Z}
(h^{-1}\cdot Z)_\ell
\;:=\;
K_\ell^{-1}\, Z_\ell\, K_{\ell-1},
\qquad \ell=1,\dots,L,
\end{equation}
(and $A$ acts trivially on $\mathcal Z$).
Let $G\times_{H_r}\mathcal Z$ denote the quotient of $G\times\mathcal Z$ by this right action.

Define
\begin{equation}\label{eq:Phi-def}
\Phi:\; G\times \mathcal Z \to \Omega,
\qquad
\Phi(g,Z)\;:=\; g\cdot \iota(Z),
\end{equation}
with $\iota$ as in \eqref{eq:iota-embed} and the $G$-action as in \eqref{eq:gauge-action}.
Then:
\begin{enumerate}
\item $\Phi(G\times\mathcal Z)=\Omega_r$.
\item $\Phi$ is $H_r$-invariant for the action \eqref{eq:Hr-action-on-GxZ}, hence descends to a well-defined map
\[
\overline\Phi:\; G\times_{H_r}\mathcal Z \to \Omega_r,\qquad \overline\Phi([g,Z])=\Phi(g,Z).
\]
\item $\overline\Phi$ is a bijection. Consequently, as sets,
\begin{equation}\label{eq:Omega_r-isomorphic}
\Omega_r \;\cong\; G\times_{H_r}\mathcal Z.
\end{equation}
\end{enumerate}
\end{proposition}

\begin{proof}
Step 1 (Image equals $\Omega_r$).
By definition \eqref{eq:Omega_r-def},
\[
\Omega_r \;=\; \{g\cdot \iota(Z): g\in G,\; Z\in\mathcal Z\}.
\]
But $\Phi(g,Z)=g\cdot \iota(Z)$ by \eqref{eq:Phi-def}, hence $\Phi(G\times\mathcal Z)=\Omega_r$.

\smallskip
Step 2 ($H_r$-invariance).
Take $(g,Z)\in G\times\mathcal Z$ and $h\in H_r$.
Using \eqref{eq:Hr-action-on-GxZ}--\eqref{eq:Hr-action-on-Z},
\[
\Phi\bigl((g,Z)\cdot h\bigr)
\;=\;
\Phi\bigl(gh,\;h^{-1}\cdot Z\bigr)
\;=\;
(gh)\cdot \iota(h^{-1}\cdot Z).
\]
By the group action property \eqref{eq:gauge-action},
\[
(gh)\cdot \iota(h^{-1}\cdot Z)
\;=\;
g\cdot\bigl(h\cdot \iota(h^{-1}\cdot Z)\bigr).
\]
Thus it suffices to show
\begin{equation}\label{eq:conjugacy-iota}
h\cdot \iota(h^{-1}\cdot Z) \;=\; \iota(Z).
\end{equation}
Write $h_\ell=\begin{pmatrix}A&0\\0&K_\ell\end{pmatrix}$ as in \eqref{eq:Hr-form}.
Then, using \eqref{eq:gauge-action} and \eqref{eq:iota-embed},
\[
...
h_{\ell-1}^{-1}.
\]
Since $h_{\ell-1}^{-1}=\begin{pmatrix}A^{-1}&0\\0&K_{\ell-1}^{-1}\end{pmatrix}$ and $(h^{-1}\cdot Z)_\ell=K_\ell^{-1}Z_\ell K_{\ell-1}$ from \eqref{eq:Hr-action-on-Z}, we compute the block product explicitly:
\[
h_\ell
\begin{pmatrix}
I_r & 0\\
0 & K_\ell^{-1}Z_\ell K_{\ell-1}
\end{pmatrix}
h_{\ell-1}^{-1}
=
\begin{pmatrix}
...
\iota(Z)_\ell.
\]
This proves \eqref{eq:conjugacy-iota}, hence
$\Phi((g,Z)\cdot h)=\Phi(g,Z)$ for all $h\in H_r$. Therefore $\Phi$ is $H_r$-invariant and descends
to a well-defined map $\overline\Phi:G\times_{H_r}\mathcal Z\to \Omega_r$.

\smallskip
Step 3 (Surjectivity of $\overline\Phi$).
By Step 1, every element of $\Omega_r$ has the form $\Phi(g,Z)$ for some $(g,Z)$.
Thus every element of $\Omega_r$ is in the image of $\overline\Phi$.

\smallskip
Step 4 (Injectivity of $\overline\Phi$).
Assume $\overline\Phi([g,Z])=\overline\Phi([g',Z'])$.
Then $\Phi(g,Z)=\Phi(g',Z')$, i.e.
\begin{equation}\label{eq:equal-orbits}
g\cdot \iota(Z) \;=\; g'\cdot \iota(Z').
\end{equation}
Let $h:=g^{-1}g'\in G$. Applying $g^{-1}$ to \eqref{eq:equal-orbits} and using the group action property gives
\begin{equation}\label{eq:h-sends-slice-point}
\iota(Z) \;=\; h\cdot \iota(Z').
\end{equation}
In particular, $h$ maps the slice point $\iota(Z')\in S_r$ back into the slice $S_r$.
We now show that this forces $h\in H_r$ and that $Z$ and $Z'$ are related by \eqref{eq:Hr-action-on-Z}.

First, because \eqref{eq:h-sends-slice-point} holds for some $\iota(Z')\in S_r$ and both sides lie in $S_r$,
the element $h$ belongs to the setwise stabilizer of $S_r$, i.e. $h\in H_r$ by Definition~\ref{def:Hr}.
Hence $h$ has the block form \eqref{eq:Hr-form}:
$h_\ell=\begin{pmatrix}A&0\\0&K_\ell\end{pmatrix}$.

Second, compute \eqref{eq:h-sends-slice-point} componentwise using \eqref{eq:gauge-action}:
\[
\iota(Z)_\ell
=
\bigl(h\cdot \iota(Z')\bigr)_\ell
=
h_\ell
\begin{pmatrix}
I_r & 0\\
...
h_{\ell-1}^{-1}.
\]
Substituting $h_\ell=\begin{pmatrix}A&0\\0&K_\ell\end{pmatrix}$ and $h_{\ell-1}^{-1}=\begin{pmatrix}A^{-1}&0\\0&K_{\ell-1}^{-1}\end{pmatrix}$ and multiplying as in Step 2 yields
\[
\begin{pmatrix}
I_r & 0\\
0 & Z_\ell
...
\begin{pmatrix}
I_r & 0\\
0 & K_\ell Z'_\ell K_{\ell-1}^{-1}
\end{pmatrix},
\]
so
\begin{equation}\label{eq:Z-conjugacy}
Z_\ell \;=\; K_\ell Z'_\ell K_{\ell-1}^{-1}
\quad\Longleftrightarrow\quad
Z'_\ell \;=\; K_\ell^{-1} Z_\ell K_{\ell-1}.
\end{equation}
But \eqref{eq:Z-conjugacy} is exactly the relation $Z'=h^{-1}\cdot Z$ from \eqref{eq:Hr-action-on-Z}.
Therefore $(g',Z')=(gh,\;h^{-1}\cdot Z)$, i.e. $(g',Z')=(g,Z)\cdot h$ in the right action
\eqref{eq:Hr-action-on-GxZ}. Hence $[g',Z']=[g,Z]$ in $G\times_{H_r}\mathcal Z$, proving injectivity.

\smallskip
Step 5 (Conclusion).
Steps 3 and 4 show that $\overline\Phi$ is bijective, hence \eqref{eq:Omega_r-isomorphic}.
\end{proof}

\begin{remark}[Hidden-layer gauge variant]
If only hidden-layer changes of basis are allowed, replace $G$ by
$G_h:=\prod_{\ell=1}^{L-1}GL(d_\ell,\mathbb{R})$ (i.e. fix $g_0,g_L$), and replace $H_r$ by $H_{r,h}:=H_r\cap G_h$.
All statements and the proof above remain valid verbatim with this replacement.
\end{remark}
...
\sigma \;:=\; \Sigma_{YX}U_Q \in \Hom(\R^{d_L-r},\R^{d_0-r}).
\]
...
\begin{theorem}[A precise replacement for $\mathcal C_r \cong G\otimes \Gamma/H$]\label{thm:Cr-associated-bundle}
Let $\mathcal C_r\subset \Omega$ denote the set of rank-$r$ first-order critical points in the normal form of the DLN
second-order characterization, i.e.\ the set described by the existence of residual blocks $Z=(Z_1,\dots,Z_L)$ such that
\[
W_L=[U_S,U_QZ_L],\qquad
W_\ell \sim \begin{pmatrix}I_r&0\\0&Z_\ell\end{pmatrix}\ (\ell=2,\dots,L-1),\qquad
W_1=\begin{pmatrix}U_S^\top\Sigma_{YX}\Sigma_X^{-1}\\ Z_1\end{pmatrix},
\qquad
Z\in \Sigma^{d-r}_0\cap \Gamma_\sigma .
\]
Then the following hold.

\smallskip
\noindent\textbf{(1) Orbit-saturation description.}
\begin{equation}\label{eq:Cr-saturation}
\mathcal C_r \;=\; G_h\cdot \mathcal S_{r,\sigma}.
\end{equation}

\smallskip
\noindent\textbf{(2) Associated bundle model (set-level).}
Define $\Phi:G_h\times \mathcal X_{r,\sigma}\to \Omega$ by
\begin{equation}\label{eq:Phi-Cr}
\Phi(g,Z)\;:=\; g\cdot \iota(Z),
\end{equation}
where $\iota$ is defined by \eqref{eq:iota-Z}--\eqref{eq:endpoints-slice}.
Then $\Phi(G_h\times \mathcal X_{r,\sigma})=\mathcal C_r$ and $\Phi$ is invariant under the right action of
$H_{r,\sigma}$ on $G_h\times \mathcal X_{r,\sigma}$ given by
\begin{equation}\label{eq:right-action-Cr}
(g,Z)\cdot h \;:=\; (g h,\; h^{-1}\cdot Z).
\end{equation}
Hence $\Phi$ descends to a map
\[
\overline\Phi:\; G_h\times_{H_{r,\sigma}} \mathcal X_{r,\sigma} \longrightarrow \mathcal C_r.
\]
Moreover, on the locus where the slice stabilizer equals $H_{r,\sigma}$ (equivalently: where there are no extra
continuous symmetries beyond block-diagonal ones), the map $\overline\Phi$ is bijective. In particular, on the
top-dimensional stratum of $\mathcal C_r$ one has a canonical identification
\begin{equation}\label{eq:Cr-associated-bundle}
\mathcal C_r \;\cong\; G_h\times_{H_{r,\sigma}} \mathcal X_{r,\sigma}
\qquad\text{(on the principal stabilizer locus).}
\end{equation}

\smallskip
\noindent\textbf{(3) Quotient of critical points by hidden gauge.}
On the same principal stabilizer locus,
\begin{equation}\label{eq:Cr-quotient}
\mathcal C_r/G_h \;\cong\; \mathcal X_{r,\sigma}/H_{r,\sigma}.
\end{equation}
\end{theorem}
...
\smallskip
\noindent\textbf{Step 1: Proof of \eqref{eq:Cr-saturation}.}
By the assumed DLN normal form for rank-$r$ critical points, $W\in\mathcal C_r$ if and only if there exists
$Z\in\mathcal X_{r,\sigma}$ and a sequence of hidden invertible changes of basis
$g=(g_1,\dots,g_{L-1})\in G_h$ such that the transformed weights satisfy the block gauge-fixed identities
\eqref{eq:iota-Z}--\eqref{eq:endpoints-slice}. Equivalently, $g^{-1}\cdot W=\iota(Z)\in \mathcal S_{r,\sigma}$.
Therefore $W=g\cdot \iota(Z)\in G_h\cdot \mathcal S_{r,\sigma}$, proving $\mathcal C_r\subseteq G_h\cdot \mathcal S_{r,\sigma}$.
Conversely, if $W=g\cdot \iota(Z)$ with $Z\in\mathcal X_{r,\sigma}$, then $W$ has the required normal form up to the hidden
reparametrization $g$, and since $Z$ satisfies both \eqref{eq:Sigma0-dr} and \eqref{eq:Gamma-sigma}, $W$ is critical.
Thus $G_h\cdot \mathcal S_{r,\sigma}\subseteq \mathcal C_r$. This proves \eqref{eq:Cr-saturation}.

\smallskip
\noindent\textbf{Step 2: Image of $\Phi$ equals $\mathcal C_r$.}
By definition \eqref{eq:Phi-Cr}, $\Phi(g,Z)=g\cdot \iota(Z)\in G_h\cdot \mathcal S_{r,\sigma}=\mathcal C_r$ by Step 1.
Hence $\Phi(G_h\times \mathcal X_{r,\sigma})\subseteq \mathcal C_r$.
Conversely, Step 1 already shows every $W\in\mathcal C_r$ can be written $W=g\cdot \iota(Z)$ with $Z\in\mathcal X_{r,\sigma}$.
Therefore $\Phi(G_h\times \mathcal X_{r,\sigma})=\mathcal C_r$.

\smallskip
\noindent\textbf{Step 3: $H_{r,\sigma}$-invariance of $\Phi$.}
...
=(gh)\cdot \iota(h^{-1}\cdot Z).
\]
Using associativity of the group action \eqref{eq:gauge-action-crit}, this equals
\[
g\cdot\Bigl(h\cdot \iota(h^{-1}\cdot Z)\Bigr).
\]
By definition of $H_{r,\sigma}$ as the setwise stabilizer of $\mathcal S_{r,\sigma}=\iota(\mathcal X_{r,\sigma})$,
the point $\iota(h^{-1}\cdot Z)$ belongs to $\mathcal S_{r,\sigma}$ and applying $h$ brings it back to the same slice point
$\iota(Z)$. Concretely, writing each $h_\ell=\begin{psmallmatrix}A&0\\0&K_\ell\end{psmallmatrix}$ and expanding the block
multiplication exactly as in the proof of the analogous statement for $\Omega_r$, one checks
\[
h_\ell
\begin{pmatrix}I_r&0\\0&(h^{-1}\cdot Z)_\ell\end{pmatrix}
h_{\ell-1}^{-1}
=
\begin{pmatrix}I_r&0\\0&Z_\ell\end{pmatrix}
\qquad(\ell=2,\dots,L-1),
\]
and similarly at the endpoints because $h\in H_{r,\sigma}$ is required to preserve the endpoint embeddings
\eqref{eq:endpoints-slice}.
Therefore $h\cdot \iota(h^{-1}\cdot Z)=\iota(Z)$ and so $\Phi((g,Z)\cdot h)=g\cdot \iota(Z)=\Phi(g,Z)$.
This proves $H_{r,\sigma}$-invariance and yields the descended map $\overline\Phi$.

\smallskip
\noindent\textbf{Step 4: When $\overline\Phi$ is bijective and what can fail.}
Surjectivity of $\overline\Phi$ follows from Step 2.
Injectivity can fail exactly at points with \emph{extra stabilizer}: if $g\cdot \iota(Z)=g'\cdot \iota(Z')$,
then $h=g^{-1}g'$ satisfies $\iota(Z)=h\cdot \iota(Z')$. This implies $h$ stabilizes the slice point $\iota(Z')$.
On the principal stabilizer locus (where the stabilizer of $\iota(Z')$ in $G_h$ equals the block-diagonal group
$H_{r,\sigma}$ and does not jump), it follows that $h\in H_{r,\sigma}$ and necessarily $Z'=h^{-1}\cdot Z$.
Hence $[g,Z]=[g',Z']$ in $G_h\times_{H_{r,\sigma}}\mathcal X_{r,\sigma}$, proving injectivity on that locus.
This proves \eqref{eq:Cr-associated-bundle}. Taking $G_h$-quotients gives \eqref{eq:Cr-quotient}.
\end{proof}

\begin{remark}[How this corrects the conjectural formula]
The correct ``product/quotient'' description involves the \emph{hidden} gauge group $G_h$ and the
\emph{intersection} $\mathcal X_{r,\sigma}=\Sigma^{d-r}_0\cap \Gamma_\sigma$ rather than $\Gamma_d$ alone.
A fully global isomorphism $\mathcal C_r \cong G_h\times_{H_{r,\sigma}}\mathcal X_{r,\sigma}$ can fail only where stabilizers jump;
in that case the correct statement is that $\mathcal C_r$ is modeled by this associated bundle on each stabilizer stratum.
\end{remark}
